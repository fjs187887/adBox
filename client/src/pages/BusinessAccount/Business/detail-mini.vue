...
export default {
  ...
  methods: {
    startPage (data) {
      this.$router.push({ path: '/taskdetailmini', query: { tid: data.tid } })
    },
    getbuessinfo () {
    ...
    refresh (done) {
      this.listInfo = []
      this.$http.gettask({ uid: this.uid, cut: 'task' }, data => {
    ...
    loadMore (index, done) {
      ...
      this.$http.gettask({ uid: this.uid, cut: 'task', page: index }, data => {
        if (data.code === 0 && data.data.list.length >= 0) {

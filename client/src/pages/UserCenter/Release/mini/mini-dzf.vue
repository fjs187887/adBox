        ...
        <q-infinite-scroll ref="listScoll" @load="loadMore" :offset="250" >
          <q-pull-to-refresh @refresh="refresh">
            ...
            <div class="row item" v-for="dataInfo in listInfo" :key="dataInfo.id" >
              ...
              <!-- 中间头像 (标题 价格) -->
              <div class="row col-12 cenBox" @click="startPage(dataInfo,'/releaseminidetail',dataInfo.tid)">
                <div class="col-12">
                  <div class="cenImgBox">
              ...
              <div class="column items-end" style="width: 100%">
                <div class="col row" v-if="dataInfo.status == 0">
                  ...
                  <q-btn class="col" outline label="编辑" :dense="true" @click="startPage(dataInfo,'/releasetaskmini',true)"></q-btn>
                  <q-btn class="col" outline label="支付" :dense="true" @click="toPay(dataInfo.order_sn)"></q-btn>
                </div>
                <div class="col" v-else-if="dataInfo.status == 4">
                  <q-btn outline label="删除" :dense="true" @click="candel(dataInfo.id,-1)"></q-btn>
                </div>
                <div class="col" v-else>
                  <q-btn outline label="再次发布" :dense="true" @click="startPage(dataInfo,'/releasetaskmini')"></q-btn>
                </div>
              </div>
            </div>
          </q-pull-to-refresh>
...
export default {
  ...
  methods: {
    ...
    getData () {
      this.loadings = true
      this.listInfo = []
      this.$http.task({ cut: 'task', status: 0 }, data => {
        this.loadings = false
    ...
    loadMore (index, done) {
      ...
      index = index + 1
      this.$http.task({ cut: 'task', status: 0, page: index }, data => {
        if (data.code === 0 && data.data.length > 0) {

            ...
            <div class="row item" v-for="dataInfo in listInfo" :key="dataInfo.id" >
              ...
              <div class="column items-end" style="width: 100%">
                <div class="col row" v-if="dataInfo.status == 0">
                  ...
                  <q-btn class="col" outline label="编辑" :dense="true" @click="startPage(dataInfo,'/releasetaskminimini',true)"></q-btn>
...
export default {
  ...
  methods: {
    ...
    getData () {
      this.loadings = true
      this.listInfo = []
      this.$http.task({ cut: 'task', status: 2 }, data => {
        this.loadings = false
    ...
    loadMore (index, done) {
      ...
      index = index + 1
      this.$http.task({ cut: 'task', status: 2, page: index }, data => {
        if (data.code === 0 && data.data.length > 0) {

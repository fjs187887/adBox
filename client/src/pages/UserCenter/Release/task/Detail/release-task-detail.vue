...
<template>
  <q-layout class="animated">
    ...
    <div class="row menuBox">
      <q-tabs class="col-8" v-model="tab" indicator-color="black" active-color="black" >
...
<!-- 1广告详情 -->
    <div v-show="tab==1">
      <div class="contentBox full-width">
        ...
        <div class="row item">
          <p class="col-3">广告标题：</p>
          <span class="col">{{contact.title}}</span>
        </div>
        <div class="row item">
          <p class="col-3">内容链接：</p>
          <span class="col">{{contact.link}}</span>
        </div>
        <div class="row item">
          <p class="col-3">封面图：</p>
        ...
        <div class="row item">
          <p class="col-3">广告预算：</p>
          <span class="col">{{contact.budget}}元</span>
        </div>
        <div class="row item">
          <p class="col-3">单次点击：</p>
          <span class="col">{{contact.uv_income}}元</span>
        </div>
        <div class="row item">
          <p class="col-3">每人最多得：</p>
          <span class="col">{{contact.max_income}}元</span>
        </div>
        <div class="row item">
      ...
      <div class="line"></div>
      <!-- 分享要求 -->
      <div class="row btBorder bottomBox" v-if="contact.task_step_explain">
        <p class="row col-12">分享要求</p>
        <div v-html="contact.task_step_explain"></div>
      </div>
      <!-- 按钮 -->
...
<!-- 2数据报表 -->
    <div v-show="tab==2">
      <div class="row dataBox">
        ...
        <div class="col-4 dataItem itemBorder">
          <p class="text-primary ">{{dataTable.total || 0}}</p>
          <p>总点击数</p>
        </div>
        <div class="col-4 dataItem itemBorder">
          <p class="text-primary ">{{dataTable.click_num || 0}}</p>
          <p>付费点击数</p>
        </div>
        <div class="col-4 dataItem">
          <p class="text-primary ">{{dataTable.forward || 0}}</p>
          <p>分享数</p>
        </div>
        <!-- 总结 文字 -->
          ...
          <q-list v-for="dataInfo in shareList" :key="dataInfo.id">
            <q-item>
              <q-item-section avatar>
                <q-avatar>
                  <img :src="dataInfo.avatar || 'statics/user/avatar.png'">
                </q-avatar>
              </q-item-section>
              <q-item-section>
                <q-item-label>{{dataInfo.nickname}}</q-item-label>
                <q-item-label v-if="tabMs!=2">{{dataInfo.create_time}}</q-item-label>
              </q-item-section>
              <q-item-section v-if="tabMs==2" side>
                <q-item-label>未分享</q-item-label>
              </q-item-section>
              <q-item-section v-else side>
                <q-item-label>总点击数：{{dataInfo.total}}</q-item-label>
                <q-item-label v-if="contact.isfree!=1">收益：{{dataInfo.profit}}</q-item-label>
                <q-item-label v-if="contact.isfree!=1">付费点击数：{{dataInfo.click_num}}</q-item-label>
              </q-item-section>
            </q-item>
...
export default {
  ...
  mounted: function () {
    ...
    this.registerRightComponent(h => h('q-btn', {
      ...
      on: { click: () => { this.$router.push({ path: '/taskdetail', query: { tid: this.contact.tid } }) } } }, '广告预览'))
    // this.drawLine()
  },
  filters: {
    checkStatus (status) {
